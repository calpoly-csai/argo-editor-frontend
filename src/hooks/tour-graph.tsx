...
function updateTourGraphs(
  mutation: (graphs: TourGraphDict) => void,
  tourId?: string
) {
...
export function useTourList() {
  useListener();
  const list = useMemo(
    () => Object.entries(tourGraphs).map(([id, { title }]) => ({ title, id })),
    [tourGraphs]
  );
  return list;
}
...
export function useTour(): [TourGraph, TourUpdater] {
  useListener();
  const { tourId } = useParams<{ tourId?: string }>();
  const tour: TourGraph =
    tourId && tourGraphs[tourId]
      ? tourGraphs[tourId]
      : { title: "", description: "", startingLocation: "", locations: {} };
...
export function useTourLocation(): [TourLocation, LocationUpdater] {
  useListener();
  const { tourId, locationName } =
    useParams<{ tourId?: string; locationName?: string }>();
  if (tourId && locationName && tourGraphs[tourId]) {
    ...
    const updateLocation: LocationUpdater = (mutation) => {
      updateTourGraphs((graphs) => {
        graphs[tourId].locations[locationName] = produce(
          graphs[tourId].locations[locationName],
          mutation
        );
      }, tourId);
    };
    ...
    const dummyUpdater: LocationUpdater =
      (mutation) => (location: TourLocation) =>
        location;
